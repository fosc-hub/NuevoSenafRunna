  ...
  useEffect(() => {
    const fetchPersonaCompleta = async () => {
      ...
      try {
        ...
        const personaEnDemanda = personas.find((p: any) => p.persona?.id === persona.id)
                ...
                <Grid item xs={12}>
                  <Paper variant="outlined" sx={{ p: 2 }}>
                    ...
                    <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mt: 1 }}>
                      {personaCompleta.persona_enfermedades.map((enfermedad: any, idx: number) => (
                        <Chip
                          key={idx}
                          label={enfermedad.nombre || enfermedad}
                          variant="outlined"
                          color="warning"
                          size="small"
                        />
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <Paper variant="outlined" sx={{ p: 2 }}>
                  ...
                  <List dense>
                    {personaCompleta.condiciones_vulnerabilidad.map((condicion: any, idx: number) => (
                      <ListItem key={idx} divider={idx < personaCompleta.condiciones_vulnerabilidad.length - 1}>
                        <ListItemIcon>
